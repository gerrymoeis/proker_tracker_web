...
export default function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  // Check for auth_status cookie as a fallback
  const hasAuthStatusCookie = typeof document !== 'undefined' && document.cookie.includes('auth_status=authenticated');
  
  // If authenticated either through context or cookie, render children
  return (isAuthenticated || hasAuthStatusCookie) ? <>{children}</> : null;
}
