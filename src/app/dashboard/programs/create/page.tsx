...
export default function CreateProgramPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch users (potential PICs) dengan credentials untuk menyertakan cookies
        ...
        if (usersData.users && usersData.users.length > 0) {
          ...
          if (user) {
            const currentUser = usersData.users.find((u: User) => u.id.toString() === user.id.toString());
            if (currentUser) {
              console.log('Setting current user as PIC:', currentUser.name);
              setFormData(prev => ({ ...prev, picId: currentUser.id.toString() }));
            } else {
              console.log('Current user not found in users list, using first user as default');
              setFormData(prev => ({ ...prev, picId: usersData.users[0].id.toString() }));
            }
          } else {
            console.log('No authenticated user, using first user as default');
            setFormData(prev => ({ ...prev, picId: usersData.users[0].id.toString() }));
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to programs page after successful creation
      setTimeout(() => {
                    ...
                    >
                      ...
                      <SelectContent>
                        {departments.length > 0 ? (
                          ...
                          <div className="relative flex w-full h-9 items-center justify-center text-sm text-muted-foreground">
